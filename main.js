...
parsed.on('data', function (data) {
  ...
  if (/^-.*/.test(values[0])) {
    item.name = GetServerName(servername) + "-" + values[0].substring(1);
    item.deleted = true;
  } else {
    item.name = GetServerName(servername) + "-" + values[0];
  }
  //console.log(item);
  ...
  // Convert altitude
  if (item.alt) {
    item.alt = ConvertAltitude(item.alt);
  }

  // Set Icons and Layer values
  if (item.Type) {
    item.icon = SetIcon(item.Type);
    item.layer = SetLayer(item.Type);
  }

  // Set Colors and side values
  if (item.Coalition) {
    item.iconColor = SetColors(item.Coalition);
    item.lineColor = SetColors(item.Coalition);
    item.fillColor = SetColors(item.Coalition);
  ...
  // item = item.map(AddSpeed);

  // Convert Lat and Long Unit type and add D M S
  // item = item.map(({ lat, lon, ...rest }) => ({ dlat: ConvertDDToDMS(lat,0), dlon: ConvertDDToDMS(lon,1), lat, lon, ...rest }));
  //console.log(item);
  delete item.LatLongAlt;
  PushToArray(ServerArrayDiff, item);
